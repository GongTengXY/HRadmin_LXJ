    ...
    <el-form :model="formData" label-width="220px">
      <div class="block">
        ...
        <el-form-item label="员工照片">
          <imageUpload ref="staffImage"></imageUpload>
          <!-- 放置上传图片 -->
        </el-form-item>
        ...
        <el-form-item label="存档机构">
          <el-input v-model="formData.archivingOrganization" placeholder="请输入" class="inputW5"/>
        </el-form-item>
        <el-form-item label="子女状态">
          <el-input v-model="formData.stateOfChildren" placeholder="请输入" class="inputW"/>
        </el-form-item>
        ...
        <el-form-item label="有无违法违纪状态">
          <el-input v-model="formData.isThereAnyViolationOfLawOrDiscipline" placeholder="请输入" class="inputW"/>
        </el-form-item>
        <el-form-item label="有无重大病史">
          <el-input v-model="formData.areThereAnyMajorMedicalHistories" placeholder="请输入" class="inputW"/>
        </el-form-item>
      </div>
      <!-- 通讯信息 -->
      <div class="block">
        ...
        <el-form-item label="现居住地">
          <el-input v-model="formData.placeOfResidence" placeholder="请输入" class="inputW5"/>
        </el-form-item>
        <el-form-item label="通讯地址">
          <el-input v-model="formData.postalAddress" placeholder="请输入" class="inputW5"/>
        </el-form-item>
...
import {getEmployeeInfo, updatedEmployeeInfo, saveUserDetail} from '@/api/employees'
...
export default {
    ...
    methods: {
        async getEmployeeDetail() {
           this.formData = await getEmployeeInfo(this.$route.params.id)
           if(this.formData.staffPhoto) {
             this.$refs.staffImage.fileList = [{url : this.formData.staffPhoto, upload : true}]
           }
        },
        async saveUser() {            //保存员工的个人信息
          const fileList = this.$refs.staffPhoto.fileList   //读取上传组件的数据
          if(fileList.some(item => !item.upload)) {
            //查找upload中为false的，可以查到说明上传未完成
            this.$message.warning('您当前头像还未上传完成！')
            return
          }
          await saveUserDetail({...this.userInfo, staffPhoto : fileList && fileList.length ? fileList[0].url : ''})
          this.$message.success('保存个人信息成功！')
        },
        async savePersonal() {        //保存员工的基础信息
          const fileList = this.$refs.staffImage.fileList   //读取上传组件的数据
          if(fileList.some(item => !item.upload)) {
            //查找upload中为false的，可以查到说明上传未完成
            this.$message.warning('您当前照片还未上传完成！')
            return
          }
          await updatedEmployeeInfo({...this.formData, staffPhoto : fileList && fileList.length ? fileList[0].url : ''})
