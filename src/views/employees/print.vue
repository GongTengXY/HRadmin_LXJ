<template>
  <div class="dashboard-container" id="myPrint">
    <el-row type="flex" justify="end" align="center" style="margin : 10px 20px 0 0">
      ...
      <el-card>
        ...
        <div v-if="type === 'personal'">
          ...
          <table cellspacing="0" width="100%" class="tableList">
            ...
            <tr>
              <th>出生日期</th>
              <td colspan="6">{{ formData.birthday | formatDate }}</td>
            </tr>
            <tr>
              <th>最高学历</th>
            ...
            <tr>
              ...
              <th>生日</th>
              <td colspan="5">{{ formData.birthday | formatDate }}</td>
            </tr>
            <tr>
              <th>属相</th>
...
<script>
import { getEmployeeInfo, getJobInfo } from '@/api/employees'
...
export default {
  ...
  methods: {
    async getPersonalDetail() {
      this.formData = await getEmployeeInfo(this.userId) // 获取个人基本信息
    },
    async getJobDetail() {
      const userInfo = await getUserPhoto(this.userId)
